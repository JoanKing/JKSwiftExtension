...
extension StringExtensionViewController {
    ...
    @objc func test107() {
        let testString1 = "0123456789"
        let str = "234"
        guard let range = testString1.jk.range(str: str) else {
            return
        }
        JKPrint("获取某个子串在父串中的范围", "\(testString1) 中子串为：\(str) 在父串中的范围是：\(range)")
    }
}
