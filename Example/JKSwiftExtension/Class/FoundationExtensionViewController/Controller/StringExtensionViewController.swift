...
extension StringExtensionViewController {
    ...
    @objc func test1010() {
        ...
        let str = "我爱祖国"
        let newString = testString.jk.insertString(content: str, locat: 3)
        JKPrint("在任意位置插入字符串", "父字符串：\(testString)", "子字符串：\(str)", "在3的位置插入字符串：\(str) 后新的字符串为：\(newString)")
    }
}
