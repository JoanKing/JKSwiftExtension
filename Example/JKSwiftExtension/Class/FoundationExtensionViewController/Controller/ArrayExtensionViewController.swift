...
class ArrayExtensionViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        dataArray = [["安全的取某个索引的值", "数组添加数组", "数组 -> JSON字符串", "分隔数组", "随机取出数组"], ["获取数组中的指定元素的索引值", "获取元素首次出现的位置", "获取元素最后出现的位置", "获取两个数组的相同元素"], ["删除数组的中的元素(可删除第一个出现的或者删除全部出现的)", "从删除数组中删除一个数组中出现的元素，支持是否重复删除, 否则只删除第一次出现的元素", "安全移除最后一个元素"], ["删除数组中遵守NSObjectProtocol协议的元素，是否删除重复的元素", "删除一个遵守NSObjectProtocol的数组中的元素，支持重复删除"], ["数组转字符转（数组的元素是 字符串），如：[1, 2, 3] 连接器为 - ，那么转化后为 1-2-3"]]
    }
...
extension ArrayExtensionViewController {
    ...
    // MARK: 3.03、安全移除最后一个元素
    @objc func test303() {
        var children: [String] = []
        children.safeRemoveLast()
        JKPrint("result:\(children)")
    }
