...
extension UIScrollViewExtensionViewController {
    ...
    //MARK: 1.4、获取内容截图
    @objc func test104() {
        let image = self.tableView.jk.captureLongScreenshot()
        image.jk.saveImageToPhotoAlbum { isSuccess in
            JKPrint("保存结果：\(isSuccess ? "成功" : "失败")")
        }
    }
